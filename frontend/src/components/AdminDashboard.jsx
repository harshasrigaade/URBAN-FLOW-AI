...
export default function AdminDashboard() {
  const { adminMetrics, adminUsers } = useMobility();
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-4xl mx-auto pb-12">
      ...
      <div className="glass-panel p-5 rounded-2xl flex flex-col gap-4">
        <h3 className="text-sm font-bold text-slate-200 border-b border-darkBg-border pb-2 flex items-center gap-2">
          <Sparkles className="w-4 h-4 text-brand-neonCyan" />
          Registered Citizens Database
        </h3>
        
        <div className="overflow-x-auto max-h-60">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-darkBg-border text-slate-400 font-bold uppercase tracking-wider text-[10px]">
                <th className="py-2.5 px-3">ID</th>
                <th className="py-2.5 px-3">Name</th>
                <th className="py-2.5 px-3">Email</th>
                <th className="py-2.5 px-3">Role</th>
                <th className="py-2.5 px-3 text-right">Sustainability Points</th>
              </tr>
            </thead>
            <tbody>
              {adminUsers && adminUsers.map((user) => (
                <tr key={user.id} className="border-b border-darkBg-border/50 hover:bg-slate-800/30 transition-all text-slate-300">
                  <td className="py-2.5 px-3 font-semibold text-slate-400">#{user.id}</td>
                  <td className="py-2.5 px-3 font-bold text-slate-100">{user.name}</td>
                  <td className="py-2.5 px-3 text-brand-neonCyan">{user.email}</td>
                  ...
                  <td className="py-2.5 px-3 text-right font-black text-slate-100">{user.sustainability_points} XP</td>
                </tr>
              ))}
            </tbody>
          </table>
