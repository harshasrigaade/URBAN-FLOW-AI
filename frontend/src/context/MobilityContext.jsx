...
export const MobilityProvider = ({ children }) => {
  ...
  const [activeSafetyScore, setActiveSafetyScore] = useState(null);
  ...
  const register = async (name, email, password, phone = null) => {
    ...
    try {
      await api.register(cleanName, cleanEmail, password, cleanPhone);
    } catch (e) {
  ...
  const calculateRoute = async (start, end, preference = routingPreference) => {
    ...
    try {
      const data = await api.planRoute(start.lat, start.lng, end.lat, end.lng, preference);
      setRoutesData(data);
      ...
      const costData = await api.getCostCompare(distance);
  ...
  const checkSafetyScore = async (lat, lng) => {
    ...
    try {
      const scoreData = await api.getSafetyScore(lat, lng);
      setActiveSafetyScore(scoreData);
      return scoreData;
    } catch (e) {
